...
def run_dyn(res_in, time_in, filt_in, filt_scale, indir, odir, opt, ingrid,
            start_point=0, time_name = 'time_series_600_600', vapour=True, c_the=False):
    ...
    for j, new_filter in enumerate(filter_list):
        ...
        S_ij_abs_S_hat_filt = sf.filter_field(S_ij_abs_S, filtered_data, opt, new_filter)
        ...
        abs_S_dth_e_dx_filt = sf.filter_field(abs_S_dth_e_dx, filtered_data, opt, new_filter)
        ...
        abs_S_dqv_dx_filt = sf.filter_field(abs_S_dqv_dx, filtered_data, opt, new_filter)
...
def Cs(indir, dx_bar, dx_hat, file_save_to, ingrid, save_all=2, reaxes=False):
    ...
    if save_all==1:
        ...
        save_field(ds_save, LM_prof)
        save_field(ds_save, MM_prof)
        save_field(ds_save, Lij_prof)
        save_field(ds_save, Mij_prof)
...
def C_scalar(scalar, indir, dx_bar, dx_hat, file_save_to, ingrid, save_all = 2, axisfix=False):
    ...
    if save_all == 2:
        ...
        C_scalar_sq_prof, C_scalar_prof, HR_prof, RR_prof, Hj_prof, Rj_prof, HR_field, RR_field = dyn.C_scalar_profiles(Hj, Rj, return_all=2)
        ...
        HR_prof = xr.DataArray(HR_prof[...], coords={'time': times, 'zn': zn_s},
                               dims=['time', "zn"], name=f'HR_{scalar_name}_prof')
        ...
        HR_field = xr.DataArray(HR_field, coords={'time': times, 'x_p': x_s, 'y_p': y_s, 'zn': zn_s},
                                     dims=["time", "x_p", "y_p", "zn"], name=f'HR_{scalar_name}_field')
        ...
        save_field(ds_save, Hj_prof)
        save_field(ds_save, Rj_prof)
    ...
    if save_all == 3:
        ...
        C_scalar_sq_field = xr.DataArray(C_scalar_sq_field[...],
                                     coords={'time': times, 'x_p': x_s, 'y_p': y_s, 'zn': zn_s},
                                     dims=["time", "x_p", "y_p", "zn"], name=f'C_{scalar_name}_sq_field')
        ...
        if len(Hj.shape) == 5:
            ...
            Rj = xr.DataArray(Rj_av[...],
                              coords={'time': times, 'j': j_s, 'x_p': x_s, 'y_p': y_s, 'zn': zn_s},
