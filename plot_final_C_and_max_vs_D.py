...
def plot_C_all_Deltas(Cs, Cth, Cqt, z, z_i, interp=False, C_sq_to_C = False,
    ...
    if interp==True:
        ...
        plt.savefig(plotdir + f'C{name}_{what_plotting}prof_scaled_interp_z.png', bbox_inches='tight')
    else:
        ax[0].set_ylabel("zn/z$_{ML}$", fontsize=16)
        plt.savefig(plotdir + f'C{name}_{what_plotting}prof_scaled_zn.png', bbox_inches='tight')
    plt.close()
...
def plot_condit_C_each_Deltas(Cs_in, Cth_in, Cqt_in, z, z_i, interp=False, C_sq_to_C = True,
    ...
    for it in range(np.shape(Cs_in)[1]):
        ...
        if interp==True:
            ax[0].set_ylabel("z/z$_{ML}$", fontsize=16)
            plt.savefig(plotdir + f'C{name}condit_prof_D={deltas[it]}_{what_plotting}_scaled_interp_z.png', bbox_inches='tight')
        else:
            ax[0].set_ylabel("zn/z$_{ML}$", fontsize=16)
            plt.savefig(plotdir + f'C{name}condit_prof_D={deltas[it]}_{what_plotting}_scaled_zn.png', bbox_inches='tight')
        plt.close()
...
def plot_max_C_l_vs_Delta(Cs_max_in, Cth_max_in, Cqt_max_in, y_ax):
    ...
    plt.savefig(plotdir+f'max_{y_ax}_{what_plotting}_prof.png', bbox_inches='tight')
    plt.close()
