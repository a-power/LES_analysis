...
#def Cs_av_levels_neg(L_ij, M_ij):
...
#             #C_s += 0.5 * ( (L_ij[it,:,:,:] * M_ij[it,:,:,:]) / M_ij[it,:,:,:]**2 )                  
...
#         Cs_av = (0.5*(LM_av / MM_av))**(1/2)
        
#     return Cs_av, LM_av, MM_av   
...
#     z_num = len(C_s[0,0,:])
#     horiz_num_temp = len(C_s[0,:,0])
#     horiz_num = horiz_num_temp**2

#     C_s_flat = C_s.reshape(horiz_num,z_num)
#     Cs_av = np.zeros(z_num)
...
#             if C_s_flat[i,k] > 0:
#                 Cs_pos_sum += C_s_flat[i,k]**(1/2)
#                 n = n+1
#             if n != 0:    
#                 Cs_av[k] = Cs_pos_sum/n