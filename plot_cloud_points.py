...
plt.figure(figsize=(6,7))
plt.plot(-26, -29)
plt.semilogx(np.mean(cloud_count_2, axis=0), z/z_i, label = '$\\Delta = 40}$m')
plt.semilogx(np.mean(cloud_count_4, axis=0), z/z_i, label = '$\\Delta = 80}$m')
plt.semilogx(np.mean(cloud_count_8, axis=0), z/z_i, label = '$\\Delta = 160}$m')
plt.semilogx(np.mean(cloud_count_16, axis=0), z/z_i, label = '$\\Delta = 320}$m')
plt.semilogx(np.mean(cloud_count_32, axis=0), z/z_i, label = '$\\Delta = 640}$m')
plt.semilogx(np.mean(cloud_count_64, axis=0), z/z_i, label = '$\\Delta = 1280}$m')
plt.xlabel(f"Number of Grid Points with 'Cloud', time averaged", fontsize=16)
plt.ylabel("z/z$_{ML}$", fontsize=16)
plt.legend(fontsize=12, loc='upper right')
plt.xlim(left = 0)
