...
plt.plot(cloud_count_2[0, :], z/z_i, label = '$\\Delta = 40}m$')
plt.plot(cloud_count_4[0, :], z/z_i, label = '$\\Delta = 80}m$')
plt.plot(cloud_count_8[0, :], z/z_i, label = '$\\Delta = 160}m$')
plt.plot(cloud_count_16[0, :], z/z_i, label = '$\\Delta = 320}m$')
plt.plot(cloud_count_32[0, :], z/z_i, label = '$\\Delta = 640}m$')
plt.plot(cloud_count_64[0, :], z/z_i, label = '$\\Delta = 1280}m$')
plt.xlabel("Number of Grid Points with 'Cloud'", fontsize=16)
plt.ylabel("z/z$_{ML}$", fontsize=16)
plt.legend(fontsize=12, loc='upper right')
plt.xlim(left = 0)
plt.ylim(bottom = 0)
plt.savefig(plotdir+'cloud_count_prof_scaled_t0.png', pad_inches=0)
plt.close()
