...
def plotfield(field, x_or_y, axis_set, data_field_list, set_percentile, data_cl_list, x_range, y_range, z_range):
    ...
    for i in range(len(data_field_list)):
        ...
        if field == 'Cs_field':
            LM_field = np.mean(data_field_list[i]['LM_field'].data[:, x_range[0]:x_range[1], y_range[0]:y_range[1], z_range[0]:z_range[1]], axis=0)
            MM_field = np.mean(data_field_list[i]['MM_field'].data[:, x_range[0]:x_range[1], y_range[0]:y_range[1], z_range[0]:z_range[1]], axis=0)
            data_field_sq = LM_field/MM_field
            data_field = dyn.get_Cs(data_field_sq)

        elif field == 'Cth_field':
            HR_field = np.mean(data_field_list[i]['HR_th_field'].data[:, x_range[0]:x_range[1], y_range[0]:y_range[1], z_range[0]:z_range[1]], axis=0)
            RR_field = np.mean(data_field_list[i]['RR_th_field'].data[:, x_range[0]:x_range[1], y_range[0]:y_range[1], z_range[0]:z_range[1]], axis=0)
            data_field_sq = HR_field/RR_field
            data_field = dyn.get_Cs(data_field_sq)

        elif field == 'Cqt_field':
            HR_field = np.mean(data_field_list[i]['HR_q_total_field'].data[:, x_range[0]:x_range[1], y_range[0]:y_range[1], z_range[0]:z_range[1]], axis=0)
            RR_field = np.mean(data_field_list[i]['RR_q_total_field'].data[:, x_range[0]:x_range[1], y_range[0]:y_range[1], z_range[0]:z_range[1]], axis=0)
            data_field_sq = HR_field/RR_field
            data_field = dyn.get_Cs(data_field_sq)
        ...
        plt.ylabel("z")
        og_ytic = plt.yticks()
        plt.yticks(og_ytic[0], np.linspace((z_range[0])*(20/1000), (z_range[1])*(20/1000), len(og_ytic[0])))
...
plotfield(**RjRj_qt_options, **set_focus)

plotfield(**Cqt_options, **set_focus)