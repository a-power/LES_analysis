...
if case=='BOMEX':
    ...
    time = '14400'
    if filt_num == 1:
        homedir = f'/work/scratch-pw3/apower/BOMEX/first_filt/{folder}/LM/C_profs/BOMEX_m0020_g0800_all_14400_C_cond_profs_'
    ...
    if filt_num == 1:
        homedir = f'/work/scratch-pw3/apower/ARM/first_filt/{folder}/LM/C_profs/diagnostics_3d_ts_{time}_C_cond_profs_'
        res = ['2D', '4D', '8D', '16D', '32D', '64D']
        #res = ['25_50', '50_100', '100_200']
    elif filt_num == 2:
        homedir = f'/work/scratch-pw3/apower/ARM/second_filt/{folder}/LM/C_profs/diagnostics_3d_ts_{time}_C_cond_profs_'
        if beta == 0:
...
for r_ind, r in enumerate(res):
    ...
    for p_ind, p in enumerate(partitions):
        for s in range(len(denom)):
            ...
            LM = [f'{numer[s]}_{p}'].data[...]
            MM = [f'{denom[s]}_{p}'].data[...]
