...
def plotfield(plot_dir, field, x_or_y, axis_set, data_field_in, set_percentile, contour_field_in, t_av_or_not,
    ...
    for i in range(len(deltas)):
        ...
        for t_set in range(len(t_av_or_not)):
            ...
            ax1.set_xticks(og_xtic[0])#, labels = np.linspace(start, end, len(og_xtic[0])))
            ax1.xaxis.set_major_formatter(np.linspace(start, end, len(og_xtic[0])), FormatStrFormatter('%.1f'))
