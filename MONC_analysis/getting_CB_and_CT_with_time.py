...
def get_CT_and_CB(ts_of_cloud_frac_prof, len_ts, len_zn_in):
    ...
    CT_nan_index = np.argwhere(np.isnan(CT_ref_25m))
    CB_nan_index = np.argwhere(np.isnan(CB_ref_25m))

    print('shape of CT_nan_index and CB_nan_index is ', np.shape(CT_nan_index), ' and ', np.shape(CB_nan_index))
    ...
    # temp_CT = savgol_filter((CT_ref_25m[CT_nan_end+1:]), 31, 5)
    # temp_CB = savgol_filter((CB_ref_25m[CB_nan_end+1:]), 31, 5)
    #
...
if plot_choice == 'HCs_HCsSA':
    ...
    for i in range(6):
        ...
        if i >= 3:
            plt.plot(plot_ref_tstamps, CB_mean_height_ts[i, :], colour_cycle[i % 3], linewidth=1)
            plt.plot(plot_ref_tstamps, CT_mean_height_ts[i, :], colour_cycle[i % 3], linewidth=1,
    ...
    CB_mean_height_ts = np.zeros( (8, 640) )
    CT_mean_height_ts = np.zeros( (8, 640) )
    cloud_init = np.zeros(7)
    ...
    for n in range(7):
        if n == 0: #unalt
            ...
            path_in = path_MONC_alt_HCs + f'{res_in_m}m/HCth_L/SA/'
    ...
    for i in range(7):
        ...
        # elif i == 1:
        #     plt.plot(plot_ref_tstamps, CB_mean_height_ts[i, :], colour_cycle[i], linestyle='--')#, marker='*')
        #     plt.plot(plot_ref_tstamps, CT_mean_height_ts[i, :], colour_cycle[i], linestyle='--',
        #              label=f'Smag 0.137')
        #     plt.vlines(plot_ref_tstamps[int(cloud_init[i])], 250, 1500, colour_cycle[i], linestyle='--')
        # elif i == 2:
        #     plt.plot(plot_ref_tstamps, CB_mean_height_ts[i, :], colour_cycle[i], linestyle='--')#, marker='*')
        ...
        #     plt.vlines(plot_ref_tstamps[int(cloud_init[i])], 250, 1500, colour_cycle[i], linestyle='--')
            ...
            plt.plot(plot_ref_tstamps, CB_mean_height_ts[i, :], colour_cycle[i]) #, linestyle='--')#, linewidth=2)
            plt.plot(plot_ref_tstamps, CT_mean_height_ts[i, :], colour_cycle[i],
                     label='$C_s$ prof')
            plt.vlines(plot_ref_tstamps[int(cloud_init[i])], 250, 1500, colour_cycle[i])
        elif i == 4:
            plt.plot(plot_ref_tstamps, CB_mean_height_ts[i, :], colour_cycle[i])#, linestyle='--', marker='x')
            plt.plot(plot_ref_tstamps, CT_mean_height_ts[i, :], colour_cycle[i],
                     label='S-A $C_s$ prof')
            plt.vlines(plot_ref_tstamps[int(cloud_init[i])], 250, 1500, colour_cycle[i])
        elif i == 5:
    ...
    save_name = f'ARM_cloud_top_and_base_ts_D_{res_in_m}_Cth_L_SA'
        ...
        # ARM_cloud_top_and_base_ts_D_{res_in_m}_Cs_and_C_th_cases
